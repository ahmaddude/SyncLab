...
function stripInlineColors(html) {
  ...
  const nodes = doc.body.querySelectorAll('[style]');
  nodes.forEach((el) => {
    const kept = (el.getAttribute('style') || '')
      ...
      .filter((s) => {
        ...
        return s && prop !== 'color' && prop !== 'background-color' && prop !== 'background';
      });
...
const TOOLBAR_BUTTONS = [  { cmd: 'bold', label: 'B', style: 'font-bold' },
...
export default function DocumentEditor() {
  ...
  useEffect(() => {
    api.get(`/documents/${id}`).then((data) => {
      setDoc(data);
      setTitle(data.title);
      if (editorRef.current) editorRef.current.innerHTML = stripInlineColors(data.content);
      setLoading(false);
  ...
  function handleInput() {
    ...
    const content = stripInlineColors(editorRef.current.innerHTML);
    const s = socketRef.current;
  ...
  if (!doc) {
    return (
      <div className="min-h-screen">
        <div className="max-w-4xl mx-auto px-4 py-8"><p className="text-gray-400">Document not found.</p></div>
      </div>
    );
  }
      ...
      <div className="flex-1 min-w-0 overflow-y-auto">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6">
          <div className="mb-2 text-sm text-gray-400">
            ...
            <Link to={`/workspaces/${doc.workspace?._id || doc.workspace}`} className="hover:text-gold transition-colors">
              Workspace
            </Link>
            <span className="mx-2 text-gray-500">/</span>
          ...
          <div className="flex items-center justify-between mb-4">
            <input type="text" value={title} onChange={handleTitleChange}
              className="text-3xl font-bold bg-transparent border-none focus:outline-none text-white w-full font-heading tracking-tight"
              placeholder="Untitled" />
            <div className="flex items-center gap-2 shrink-0 ml-4">
              <button onClick={() => setShowAI(!showAI)}
                className={`flex items-center gap-2 px-4 py-2.5 text-sm transition-all ${showAI ? 'bg-gold/10 text-gold border border-gold/30' : 'border border-line bg-ink-900 text-gray-300 hover:bg-ink-850'}`}>
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>
                AI Assistant
              </button>
              <button onClick={saveNow} disabled={saving}
                className="flex items-center gap-2 px-4 py-2.5 text-sm bg-gold text-ink-950 hover:bg-gold-hover font-semibold disabled:opacity-50 transition-all">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4" /></svg>
                {saving ? 'Saving...' : 'Save'}
              </button>
              <button onClick={downloadDoc}
                className="flex items-center gap-2 px-4 py-2.5 text-sm border border-line bg-ink-900 text-gray-300 hover:bg-ink-850 transition-all">
          ...
          <div className="bg-ink-850 border border-line rounded-2xl overflow-hidden">
            <div className="flex items-center gap-0.5 px-3 py-2 border-b border-line flex-wrap">
              {TOOLBAR_BUTTONS.map((btn, i) =>
                btn.type === 'sep' ? (
                  <div key={i} className="w-px h-5 bg-line mx-1.5" />
                ) : (
                  <button key={i} onMouseDown={(e) => e.preventDefault()} onClick={() => execCommand(btn.cmd, btn.value)}
                    className={`px-2.5 py-1.5 text-sm text-gray-400 hover:bg-ink-800/50 hover:text-white rounded transition-all ${btn.style || ''}`}
                    title={btn.cmd}>
                    {btn.label}
                  </button>
                )
            ...
            <div ref={editorRef} contentEditable suppressContentEditableWarning
              ...
              className="p-8 min-h-[500px] focus:outline-none text-gray-200 leading-relaxed bg-ink-900"
              style={{ lineHeight: '1.8' }} />
          </div>

          <style>{`
            [contenteditable] h2 { font-size: 1.5rem; font-weight: 700; margin: 1.5rem 0 0.5rem; color: #e5e7eb; font-family: 'Source Serif 4', Georgia, serif; }
            [contenteditable] h3 { font-size: 1.25rem; font-weight: 600; margin: 1.25rem 0 0.5rem; color: #e5e7eb; font-family: 'Source Serif 4', Georgia, serif; }
            [contenteditable] p { margin: 0.5rem 0; color: inherit; }
            ...
            [contenteditable] span, [contenteditable] div, [contenteditable] li { color: inherit; }
            [contenteditable] ::selection { background: #d4af37; color: #0b0f19; }
          `}</style>
        </div>
      </div>
